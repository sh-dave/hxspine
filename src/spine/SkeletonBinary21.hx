...
class SkeletonBinary21 {
	...
	public function readSkeletonData(binary:Bytes):SkeletonData {
		...
		skeletonData.x = 0; //input.readFloat();
		skeletonData.y = 0; //input.readFloat();
		skeletonData.width = input.readFloat();
		skeletonData.height = input.readFloat();

		var nonessential = input.readBoolean();
		if (nonessential) {
			skeletonData.fps = 60;//input.readFloat();
		...
		for (i in 0...n) {
			...
			final parentIndex = input.readInt(true) - 1;
			var parent = parentIndex != -1 ? skeletonData.bones[parentIndex] : null;
			...
			data.shearX = 0;//input.readFloat();
			data.shearY = 0;//input.readFloat();
			data.length = input.readFloat() * scale;
			final flipX = input.readBoolean(); // TODO (DK) tranlate to shear or something?
			final flipY = input.readBoolean(); // TODO (DK) tranlate to shear or something?

			data.transformMode = Normal;//cast input.readInt(true);
			data.skinRequired = false;//input.readBoolean();

			final inheritScale = input.readBoolean(); // TODO (DK)
			final inheritRotation = input.readBoolean(); // TODO (DK)
		...
		for (i in 0...n) {
			...
			data.blendMode = input.readBoolean() ? Additive : Normal;
		...
		// Default skin.
		var defaultSkin = this.readSkin(input, "default", nonessential, skeletonData);
		if (defaultSkin != null) {
		...
		// Skins.
		{
			...
			while (i < n)
				skeletonData.skins[i++] = this.readSkin(input, input.readString(), nonessential, skeletonData);
		}
	...
	function readSkin(input:BinaryInput, skinName: String, nonessential:Bool, skeletonData: SkeletonData):Skin {
		var skin = null;
		...
		final skin = new Skin(skinName);
		for (i in 0...slotCount) {
			final slotIndex = input.readInt(true);

			for (ii in 0...input.readInt(true)) {
				final name = input.readString();
				final att = readAttachment(input, skeletonData, skin, slotIndex, name, nonessential);
				skin.setAttachment(slotIndex, name, att);
			}
		}
...
// TODO (DK) SkinnedMesh
			case other:
				trace('reading `$other` is not implemented yet');
...
private class BinaryInput {
	...
	public function readString():String {
		...
		if (charIndex < charCount) {
			trace('foo');
			// ReadUtf8_slow(input, charCount, charIndex, b);
		}
		return chars;
	}
